...
Our recent work suggests that implicit recalibration is [insensitive to state noise](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1011951). However, this system shows unique effects in response to contextual uncertainty. For example, learning is faster but less durable when the environment is volatile; reversal learning is slower than original learning; and relearning is slower, rather than faster, when re-exposed to the same perturbation. We proposed a [Cerebellar Population Coding model](https://pubmed.ncbi.nlm.nih.gov/37461557/) that offers a unified explanation for these contextual effects without invoking contextual inference. The model is a simple two-layer network corresponding to the cerebellar cortex and deep cerebellar nuclei, and contextual effects emerge from the population activation within the model.
...
Our work on [target-specific bias](https://elifesciences.org/reviewed-preprints/100715) shows that the pattern reflects a [mismatch](https://www.nature.com/articles/s41598-020-76220-0) between the proprioceptive space that encodes hand position and the visual space used for target localization. Modeling suggests that participants perceive the target visually, translate it into proprioceptive space, and then generate a motor plan.
...
One of my key interests is understanding the computational mechanisms of motor skill learning. We revisited a longstanding paradox: patients with retrograde amnesia (e.g., [HM](https://en.wikipedia.org/wiki/Henry_Molaison)) can learn mirror-reversed tasks, yet [recent studies show](https://journals.physiology.org/doi/pdf/10.1152/jn.00304.2021) the implicit system is surprisingly rigid—even after extended training.
